...
<!-- Title of Page -->
<?
 $this->title = Yii::t('app', 'DSP Traffic');
...
<!-- Date From - To -->
<?php echo Html::beginForm('index.php?r=traffic/dsp_traffic', 'GET'); ?>
...
<thead>
    <tr>
        ...
        <th colspan="2" style="text-align: center;"><?= Yii::t('app', 'SSP Calls') ?></th>
...
<?php foreach ($total_stat as $ssp_id => $stat) : ?>
    ...
    <tr class="clickableTR">
        <td nowrap>
         <a href="index.php?r=traffic/dsp_by_ssp_sites&ssp_id=<?=$ssp_id?>"
         title="<?= Yii::t('app', $stat['ssp_name'].' Sites Statistics') ?>">
          <?= $stat['ssp_name'].' ('.$ssp_id.')' ?>
         </a>
        </td>
        <?if (empty($format_id)) :?>
        ...
        <?endif?>
        <td <?=$style?> nowrap>
            <a href="index.php?r=traffic/dsp_call_errors&ssp_id=<?=$ssp_id?>&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>"
              title="<?= Yii::t('app', $stat['ssp_name'].' Call Errors') ?>">
...
<tfoot>
    <tr class="warning">
        ...
        <td style="text-align: right;" nowrap>
            <a href="index.php?r=traffic/dsp_call_errors&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
        ...
        <td style="text-align: right;" nowrap>
            <a href="index.php?r=traffic/dsp_auction_errors&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
        ...
        <td style="text-align: right;" nowrap>
         <a href="index.php?r=traffic/dsp_shows_stat&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
          <?= number_format($total_row['hit_ok'], 0, ',', ' '); ?>
         </a>
        </td>
        <td style="text-align: right;" nowrap>
         <a href="index.php?r=traffic/dsp_vast_errors&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
          <?= number_format($total_row['vast_error'], 0, ',', ' '); ?>
         </a>
        </td>
        
        <td <?=$style?> nowrap><?= round($total_row['bid_request']/$total_row['inner_auction'] * 100, 0); ?>%</td>
        <td <?=$style?> nowrap><?= round($total_row['hit_ok']/$total_row['bid_request'] * 100, 0); ?>%</td>
        <td <?=$style?> nowrap><?= round($total_row['click_ok']/$total_row['hit_ok'] * 100, 0); ?>%</td>
    </tr>
</tfoot>
</table>
</div>
