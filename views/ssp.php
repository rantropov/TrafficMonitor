...
<!-- Title of Page -->
<?
 $this->title = Yii::t('app', 'SSP Traffic');
...
<!-- Date From - To -->
<?php echo Html::beginForm('index.php?r=traffic/ssp_traffic', 'GET'); ?>
...
<thead>
    <tr>
        <th class="first table-sortable:string" rowspan="2" style="text-align: center;">
          <?= Yii::t('app', 'SSP') ?>
        </th>
        <th rowspan="2"></th>
...
<?php foreach ($total_stat as $ssp_id => $stat) : ?>
    ...
    <tr class="clickableTR">
        <td nowrap>
            <a href="index.php?r=traffic/ssp_by_dsp&ssp_id=<?=$ssp_id?>&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
                <?= $stat['ssp_name'].' ('.$ssp_id.')' ?>
            </a>
        </td>
        <td>
            <a href="index.php?r=traffic/ssp_sites_call&ssp_id=<?=$ssp_id?>&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
                  <img src="/images/arrow_down.png" width="18" height="18" title="Show Sites">
            </a>
        </td>
        ...
        <td <?=$style?> nowrap>
            <a href="index.php?r=traffic/ssp_unmoderated&ssp_id=<?=$ssp_id?>&date_from=<?=$from?>&date_to=<?=$to?>&format_id=<?=$format_id?>">
